 ...
 *
 * promoLinkMode per platform:
 *   'none'    = ignore promo link (Col B)
 *   'caption' = append link to caption text
 *   'comment' = post link as first comment on FB/IG (requires pages_manage_engagement)
 */
...
function smartUpload(blob, token) {
  ...
  const options = {
    method: "post",
    headers: { "Accept": "application/json", "Authorization": "Bearer " + token, "X-Api-Token": token },
    payload: { file: blob },
    followRedirects: false,
    muteHttpExceptions: true
  };

  let lastError = "Upload failed.", lastResponseCode = null, lastResponseText = "";
  for (let attempt = 1; attempt <= 3; attempt++) {
    const response = UrlFetchApp.fetch(url, options);
    const responseCode = response.getResponseCode();
    const responseText = response.getContentText();
    lastResponseCode = responseCode; lastResponseText = responseText;
    ...
    if (responseCode === 429) { lastError = "Upload rate limited. Retrying..."; Utilities.sleep(attempt * 1500); continue; }
    ...
    lastError = /<html/i.test(responseText)
      ? "Upload Failed (" + responseCode + "): The server returned an HTML page instead of JSON."
      : "Upload Failed (" + responseCode + "): " + (json.message || responseText);
...
function parseJsonResponse_(jsonText) { try { return JSON.parse(jsonText); } catch (e) { return {}; } }
...
/** Returns 'caption' | 'comment' | 'none'. Backward compat with old boolean includeLinkInCaption. */
...
/** Appends promo link to caption only when promoLinkMode === 'caption'. */
function composeCaption_(rowData, platform) {
  ...
  if (getPromoLinkMode_(platform) !== "caption" || !safeLink) return caption;
  return caption ? caption + "\n\n" + safeLink : safeLink;
}

/**
 * Builds platform-specific settings.
 * fb_comment / ig_comment injected when promoLinkMode === 'comment'.
 * Story duplicates inherit these via deep clone in sendPost().
 */
function buildPlatformSettings_(platform, rowData, mediaSpec) {
  ...
  if (handle === "tg_channel") { if (detectedType === "carousel") return null; settings.post_type = detectedType; return settings; }
  if (handle === "tt_profile") {
    if (detectedType === "text" || detectedType === "carousel") return null;
    settings.post_type = detectedType; settings.privacy_level = "SELF_ONLY"; settings.music_usage_confirmed = true; settings.tt_consent = 1;
    return settings;
  }
  if (handle === "yt_channel") {
    if (detectedType !== "video") return null;
    settings.post_type = "video"; settings.title = safeTitle || String(rowData.caption || "").slice(0, 95);
    settings.youtube_title = settings.title; settings.category = 22;
    return settings;
  }
  if (handle === "pinterest") {
    if (detectedType !== "image") return null;
    settings.post_type = "image"; settings.pinterest_title = safeTitle; settings.pinterest_board = platform.boardId || "";
  ...
  if (handle === "tumblr") { if (detectedType === "carousel") return null; settings.post_type = detectedType; return settings; }
  settings.post_type = detectedType;
  return settings;
}
...
function sendPost(rowData, config, mediaUrls, mediaSpec) {
  ...
  config.platforms.forEach(p => {
    ...
    const baseEntry = { integration: { id: p.id }, value: [{ content: composeCaption_(rowData, p) }], media: mediaUrls, settings };
    ...
    if (shouldAddStoryDuplicate_(p, mediaSpec)) {
      ...
      if (handle === "fb_page") storyEntry.settings.fb_type = "story";
      if (handle === "ig_profile") storyEntry.settings.ig_type = "story";
  ...
  const response = UrlFetchApp.fetch("https://post.devad.io/api/public/v1/posts?api_token=" + config.token, {
    method: "post", contentType: "application/json", payload: JSON.stringify({ posts }), muteHttpExceptions: true
  });
  ...
  return { skippedPlatforms, responseCode: response.getResponseCode(), serverResponse: jsonText };
}
