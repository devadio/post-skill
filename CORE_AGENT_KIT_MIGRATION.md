...
| Provider matrix | run Agent Kit `provider-matrix` / MCP `post_provider_matrix_run` before provider chunks to catch fixture-vs-validator mismatches |
...
7. Run Agent Kit `provider-matrix` / MCP `post_provider_matrix_run` where available before provider chunks.
8. Run Agent Kit `validate` / MCP `post_dry_run_validate` where available, and rely on `posts:create` preflight to block `BLOCKED` provider media-rule mismatches.
9. Preserve CORE `block_states`, Agent Kit `validation.provider_results`, warnings, and blocking reasons.
10. Add idempotency/correlation IDs to n8n and Sheets.
11. Run the template drift gate when Sheet/n8n embedded preflight maps or provider-rule fixtures change.
12. Verify external provider URLs before marking a provider/type as passed.
...
node packages\devad-post-agent\dist\cli.js provider-matrix --dry-run --provider youtube_channel
